...
const CARD_WIDTH: f32 = FORM_WIDTH + 48.0;
...
pub fn show(ui: &mut egui::Ui, s: &mut SetupState) -> Option<Event> {
    ...
    egui::CentralPanel::default_margins().show(ui, |ui| {
        ui.vertical_centered(|ui| {
            ...
            // A card on the canvas, like the work area of the editor.
            ui.horizontal(|ui| {
                ui.add_space(((ui.available_width() - CARD_WIDTH) / 2.0).max(0.0));
                theme::workspace_frame(ui.style()).inner_margin(egui::Margin::same(24)).show(ui, |ui| {
                    ui.set_width(CARD_WIDTH - 48.0);
                    ui.vertical(|ui| {
                        if let Some(e) = form(ui, s) {
                            event = Some(e);
                        }
                    });
                });
...
fn form(ui: &mut egui::Ui, s: &mut SetupState) -> Option<Event> {
    let mut event = None;
