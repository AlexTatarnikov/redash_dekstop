...
impl RedashApp {
    ...
    fn perform(&mut self, ctx: &egui::Context, effect: Effect) {
        match effect {
            ...
            Effect::LoadSchema { config, data_source_id, refresh } => {
                self.spawn(ctx, move || Event::SchemaLoaded {
                    data_source_id,
                    result: Client::new(&config).schema(data_source_id, refresh).map_err(|e| e.to_string()),
                })
            }
